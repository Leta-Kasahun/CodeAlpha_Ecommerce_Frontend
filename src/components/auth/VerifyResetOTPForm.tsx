...
export function VerifyResetOTPForm() {
  const { loading, error, handleVerifyResetOTP, handleForgotPassword } = useAuth();
  const {
    otpArray,
    countdown,
    handleOtpChange,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await handleVerifyResetOTP({
      email,
      otp: otpValue
    });
  ...
  return (
    <Card className="w-full border border-gray-200 shadow-lg">
      <CardHeader className="space-y-3 text-center">
        ...
        <CardTitle className="text-3xl font-bold text-gray-900">Verify Reset Code</CardTitle>
          ...
          <div className="text-center">
            {countdown > 0 ? (
              ...
              <button
                ...
                className="text-lg font-semibold text-[#5156D2] hover:text-[#3f43b3] transition-colors"
              >
